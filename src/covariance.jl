...
"""
Singleton type for Squared Exponential covariance kernel.

The covariance kernel is given by

```math
K(x,x') = \\exp(-\\lvert hp * (x - x') \\rvert^2)
```
...
"""
dim_hp(K::AbstractKernel, dim)

Returns the number of hyperparamaters needed for K with dim
as the dimension of the input data x.
"""
